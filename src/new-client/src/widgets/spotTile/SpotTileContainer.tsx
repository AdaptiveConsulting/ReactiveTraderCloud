...
const mapDispatchToProps = memoize(
  (dispatch: Dispatch, ownProps: SpotTileContainerOwnProps) => ({
    ...
    onNotificationDismissed: () =>
      dispatch(
        SpotTileActions.dismissNotification({ currencyPair: ownProps.id }),
      ),
    setTradingMode: (tradingMode: TradingMode) =>
      dispatch(SpotTileActions.setTradingMode(tradingMode)),
    rfq: {
      request: (rfqActionObj: RfqRequest) =>
        dispatch(SpotTileActions.rfqRequest(rfqActionObj)),
      cancel: (rfqActionObj: RfqCancel) =>
        dispatch(SpotTileActions.rfqCancel(rfqActionObj)),
      reject: (rfqActionObj: RfqReject) =>
        dispatch(SpotTileActions.rfqReject(rfqActionObj)),
      requote: (rfqActionObj: RfqRequote) =>
        dispatch(SpotTileActions.rfqRequote(rfqActionObj)),
      expired: (rfqActionObj: RfqExpired) =>
        dispatch(SpotTileActions.rfqExpired(rfqActionObj)),
      reset: (rfqActionObj: RfqExpired) =>
        dispatch(SpotTileActions.rfqReset(rfqActionObj)),
    },
...
type SpotTileContainerStateProps = ReturnType<
...
const SpotTileContainer: React.FC<SpotTileContainerProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      typeof notional !== "undefined" &&
      notional !== props.spotTileData.notional
    ) {
