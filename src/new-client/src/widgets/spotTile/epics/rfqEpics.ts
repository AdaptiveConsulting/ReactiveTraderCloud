...
function getSpotTilesDataByCurrency(
...
export const rfqRequestEpic: ApplicationEpic<{}> = (action$, state$) =>
  action$.pipe(
    ...
    filter(
      (action) =>
        !!getSpotTilesDataByCurrency(action.payload.currencyPair, state$.value),
    ),
    mergeMap((action) => {
      const cancel$ = action$.pipe(
        ...
        filter(
          (cancelAction) =>
            cancelAction.payload.currencyPair.symbol ===
            action.payload.currencyPair.symbol,
        ),
      )
  ...
  action$.pipe(
    ...
    mergeMap((action) => {
      ...
      const cancel$ = action$.pipe(
        ...
        filter(
          (cancelAction) =>
            cancelAction.payload.currencyPair.symbol === currencyPair.symbol,
        ),
      )

      return concat(
        timer(action.payload.timeout + 1000).pipe(
          map(() => rfqExpired({ currencyPair })),
        ),
