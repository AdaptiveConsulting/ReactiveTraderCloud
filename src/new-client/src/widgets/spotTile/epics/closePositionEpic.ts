...
function createTrade(
  ...
  const spotRate =
    direction === Direction.Buy ? priceData.price.ask : priceData.price.bid
...
export const closePositionEpic: ApplicationEpic = (
  action$,
  state$,
  { platform },
) => {
  ...
  const interopSubscribe$ = platform.interop.subscribe$(
    InteropTopics.ClosePosition,
  )
