  ...
  switch (action.type) {
    ...
    case TILE_ACTION_TYPES.SPOT_PRICES_UPDATE:
      ...
      return {
        ...
        historicPrices: [
          ...state.historicPrices.slice(
            startIndexUpdatePrices,
            state.historicPrices.length,
          ),
          action.payload,
        ],
      }
    case TILE_ACTION_TYPES.PRICE_HISTORY_RECEIVED:
      const startIndexPrices = Math.max(
        1,
        action.payload.length - HISTORIC_PRICES_MAX_POINTS,
      )
      return {
        ...state,
        historicPrices: action.payload.slice(
          startIndexPrices,
...
export const spotTileDataReducer = (
  ...
  switch (action.type) {
    case TILE_ACTION_TYPES.SET_NOTIONAL:
      return {
        ...state,
        [action.payload.currencyPair]: spotTileReducer(
    ...
    case TILE_ACTION_TYPES.SET_TRADING_MODE:
      return {
        ...state,
        [action.payload.symbol]: rfqTileReducer(
    ...
    case TILE_ACTION_TYPES.DISMISS_NOTIFICATION:
      const shouldDismiss =
        ...
        (state[action.payload.currencyPair] &&
          ...
          state[action.payload.currencyPair]!.lastTradeExecutionStatus!.request
    ...
    case TILE_ACTION_TYPES.SPOT_PRICES_UPDATE:
      return state[action.payload.symbol]
        ? {
            ...state,
            [action.payload.symbol]: spotTileReducer(
              state[action.payload.symbol],
              action,
            ),
          }
