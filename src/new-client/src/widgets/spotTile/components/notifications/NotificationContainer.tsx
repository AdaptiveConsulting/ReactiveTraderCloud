...
export default class NotificationContainer extends PureComponent<Props> {
  ...
  private renderNotifications: () => (
    ...
    const { lastTradeExecutionStatus, currencyPair, onNotificationDismissed } =
      this.props
    ...
    if (
      lastTradeExecutionStatus.hasError ||
      lastTradeExecutionStatus.hasWarning
    ) {
      ...
      return (style: React.CSSProperties) => (
        ...
        >
          {isError
            ...
            : lastTradeExecutionStatus.warning}
        </TileNotification>
      )
    }
    ...
    if (lastTradeExecutionStatus.trade) {
      ...
      if (lastTradeExecutionStatus.trade.status === TradeStatus.Done) {
        const { direction, notional, spotRate, valueDate } =
          lastTradeExecutionStatus.trade
