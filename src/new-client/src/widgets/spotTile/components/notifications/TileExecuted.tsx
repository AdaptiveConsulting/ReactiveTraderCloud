...
const TileExecuted: React.FC<Props> = ({
  ...
  const dealtText = `${dealtCurrency} ${numeral(notional).format(
    "0,000,000[.]00",
  )}`
  const counterText = `${counterCurrency} ${numeral(notional * rate).format(
    "0,000,000[.]00",
  )}`
  const formattedDate = `(Spt) ${DateTime.fromJSDate(date).toFormat("d MMM")}`
  return (
    <span data-qa="notification-container__tile-executed">
      You {directionText[direction]} <InverseFont>{dealtText}</InverseFont> at a
      rate of <InverseFont>{rate}</InverseFont> for{" "}
      <HeavyItalicsFont>{counterText}</HeavyItalicsFont> settling
      {"  "}
