...
export const TileNotificationStyle = styled(TileBaseStyle)<{
  accentColor: AccentColor
}>`
  color: ${({ theme }) => theme.white};
  background-color: ${({ theme, accentColor }) =>
    theme.accents[accentColor].darker};
  font-size: 0.8125rem;
  text-align: center;
  line-height: 1.5;
  display: flex;
...
const PillButton = styled(Button)<{ accentColor: AccentColor }>`
  color: ${({ theme }) => theme.white};
  background-color: ${({ theme, accentColor }) =>
    theme.accents[accentColor].base};
  border-radius: 17px;
...
const TileNotification: React.FC<Props> = ({
  ...
  return (
    ...
    >
      ...
      {tradeId && (
        <HeavyFont data-qa="tile-notification__tradeid">
          Trade ID: {tradeId}
        </HeavyFont>
      )}
