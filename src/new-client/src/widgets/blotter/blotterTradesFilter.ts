...
const tradeMatchesFilter = (
  ...
  if (!(trade as any).hasOwnProperty(filterField)) {
    console.warn(
      `Trying to filter of field ${filterField} which does not exist in 'Trade' object`,
    )
    return true
  }

  const CONTAINS_SYMBOL = filterValues.some((value) =>
    trade.symbol.includes(value),
  )
...
export function validateFilters(filters: BlotterFilters): BlotterFilters {
  return {
    count: filters.count,
    [DEALT_CURRENCY]: (filters[DEALT_CURRENCY] || []).filter(
      isMeaningfullValue,
    ),
