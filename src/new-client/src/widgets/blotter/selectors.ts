...
const getBlotterTrades = (state: GlobalState) =>
  state.blotterService && state.blotterService.trades
const getBlotterStatus = ({ compositeStatusService }: GlobalState) =>
  compositeStatusService.blotter &&
  compositeStatusService.blotter.connectionStatus
...
export const selectBlotterRowsAndFilter = (filters: BlotterFilters) =>
  createSelector(selectBlotterRows, (rows) =>
    filterBlotterTrades(rows, filters),
  )
