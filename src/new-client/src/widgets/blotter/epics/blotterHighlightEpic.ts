...
const highlightTradeEpic: ApplicationEpic = (action$, state$, { platform }) => {
  ...
  const interopObservable$ = platform.interop.subscribe$(
  ...
  return action$.pipe(
    ...
    filter(Boolean),
    map((trade) => {
      return highlightTradeAction({
        trades: [switchHighlight(trade as Trade, true)],
      })
    }),
...
const removeHighlightTradeEpic: ApplicationEpic = (
  action$,
  state$,
  { platform },
) => {
...
export const blotterHighlightEpic = combineEpics(
  highlightTradeEpic,
