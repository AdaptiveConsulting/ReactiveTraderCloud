...
type SubscribeToBlotterAction = ReturnType<
  typeof BlotterActions.subscribeToBlotterAction
>
...
export const blotterServiceEpic: ApplicationEpic<{
  ...
  const subscribeAction$ = action$.pipe(
    ofType<Action, SubscribeToBlotterAction>(
      BLOTTER_ACTION_TYPES.SUBSCRIBE_TO_BLOTTER_ACTION,
    ),
  )
