...
const formatTradeNotification = (trade: Trade, currencyPair: CurrencyPair) => ({
  ...
  valueDate: DateTime.fromJSDate(trade.valueDate, { zone: "utc" }).toFormat(
    "dd MMM",
  ),
...
function parseBlotterData(blotterData: Trades, currencyPairs: CurrencyPairMap) {
  if (
    ...
    Object.keys(blotterData).length === 0
  ) {
    return []
  }
...
export const publishBlotterToExcelEpic: ApplicationEpic = (
  action$,
  state$,
  { excelApp },
) =>
...
export const connectBlotterToNotifications: ApplicationEpic = (
  action$,
  state$,
  { platform },
) =>
  action$.pipe(
    ofType<Action, NewTradesAction>(
      BLOTTER_ACTION_TYPES.BLOTTER_SERVICE_NEW_TRADES,
    ),
    filter(() => state$.value.layoutService.blotter.visible),
    filter(
      (action) =>
        !action.payload.isStateOfTheWorld && action.payload.trades.length > 0,
    ),
    ...
    map((trade) =>
      formatTradeNotification(trade, state$.value.currencyPairs[trade.symbol]),
    ),
    tap((tradeNotification) =>
...
export const removeHighlightTradeEpic: ApplicationEpic = (
  action$,
  state$,
  { platform },
) => {
  return action$.pipe(
    ofType<Action, HighlightTradeAction>(
