...
function UtcFormatDate(date: Date) {
  const localZoneName = Info.features().zones
    ? DateTime.local().zoneName
    : "utc"

  return DateTime.fromJSDate(date, { zone: localZoneName }).toFormat(
    "dd-MMM-yyyy",
  )
}
...
export const columnDefinitions: Array<ColDef & ColCSVSettings> = [
  ...
  },
  {
    ...
    cellRenderer: ({ data }) =>
      data
        ? `<span data-qa="${data.tradeId}-tradeId">${data.tradeId}</span>`
        : "",
  },
  {
    colId: STATUS,
    ...
    cellRenderer: ({ data }) =>
      data
        ? `<span data-qa="${data.tradeId}-status">${capitalize(
            data.status,
          )}</span>`
        : "",
  },
  {
    ...
    cellRenderer: ({ data }) =>
      data
        ? `<span data-qa="${data.tradeId}-tradeDate">${UtcFormatDate(
            data.tradeDate,
          )}</span>`
        : "",
  },
  {
    ...
    cellRenderer: ({ data }) =>
      data
        ? `<span data-qa="${data.tradeId}-direction">${data.direction}</span>`
        : "",
  },
  {
    colId: SYMBOL,
    ...
    cellRenderer: ({ data }) =>
      data
        ? `<span data-qa="${data.tradeId}-symbol">${data.symbol}</span>`
        : "",
  },
  {
    ...
    cellRenderer: ({ data }) =>
      data
        ? `<span data-qa="${data.tradeId}-dealtCurrency">${data.dealtCurrency}</span>`
        : "",
  },
  {
    colId: NOTIONAL,
    ...
    cellRenderer: ({ data }) =>
      data
        ? `<span data-qa="${data.tradeId}-notional">${notionalRenderer(
            data.notional,
          )}</span>`
        : "",
  },
  {
    ...
    cellRenderer: ({ data }) =>
      data
        ? `<span data-qa="${data.tradeId}-spotRate">${data.spotRate}</span>`
        : "",
  },
  {
    colId: VALUE_DATE,
    ...
    cellRenderer: ({ data }) =>
      data
        ? `<span data-qa="${data.tradeId}-valueDate">${UtcFormatDate(
            data.valueDate,
          )}</span>`
        : "",
  },
...
export const csvExportSettings: CsvExportParams = {
  ...
  processCellCallback: (cell) => {
    const colDef = columnDefinitions.find(
      (c) => c.colId === cell.column.getColId(),
    )
