...
) => ExternalWindowProps = (key: string, tileLayout?: WindowPosition) => ({
  title: `${key} Spot`,
  config: {
    name: `${key}`,
...
export const selectSpotTiles = createSelector(
  ...
  (spotTileKeys, tilesLayout) =>
    Object.keys(spotTileKeys).map((key) => ({
      ...
      tornOff:
        tilesLayout[key] === undefined ? false : !tilesLayout[key].visible,
    })),
)
...
export const selectSpotCurrencies = createSelector(
  ...
  (spotTileKeys) =>
    Array.from(
      new Set(Object.keys(spotTileKeys).map((key) => spotTileKeys[key].base)),
    ),
)

const getExecutionStatus = ({ compositeStatusService }: GlobalState) =>
  compositeStatusService.execution &&
  compositeStatusService.execution.connectionStatus
