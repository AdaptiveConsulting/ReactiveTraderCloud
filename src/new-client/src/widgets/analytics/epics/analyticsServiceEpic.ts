...
export const publishPositionToExcelEpic: ApplicationEpic<{
  excelApp: ExcelApp
}> = (action$, state$, { excelApp }) =>
  action$.pipe(
    ofType<Action, FetchAnalyticsAction>(
...
function combineWithLatestPrices(
  ...
  return positions.map((position) => {
    ...
    return {
      ...position,
      latestAsk:
        tileData && tileData.price.ask ? tileData.price.ask : undefined,
      latestBid:
        tileData && tileData.price.bid ? tileData.price.bid : undefined,
    }
  })
}
