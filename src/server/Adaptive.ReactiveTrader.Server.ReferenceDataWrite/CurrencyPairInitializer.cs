...
namespace Adaptive.ReactiveTrader.Server.ReferenceDataWrite
{
    public class CurrencyPairInitializer
    {
        ...
        public CurrencyPairInitializer(IAggregateRepository repository)
        {
        ...
        public async Task CreateInitialCurrencyPairsAsync()
        {
            await Task.WhenAll(
                ...
                CreateCurrencyPair("GBPUSD", 4, 5, 1.6395m, "Server waits 1.5sec to execute then sends a trade done.", true),
                CreateCurrencyPair("GBPJPY", 2, 3, 167.67m, "Always rejects upon execution.", true),
                CreateCurrencyPair("EURGBP", 4, 5, 0.8312m),
                CreateCurrencyPair("USDCHF", 4, 5, 0.897m),
                CreateCurrencyPair("EURJPY", 2, 3, 139.22m, "Waits 5sec before sending a trade response (times-out).", true),
                CreateCurrencyPair("EURCHF", 4, 5, 1.2224m, "Server waits before sending a first price update."),
                CreateCurrencyPair("AUDUSD", 4, 5, 0.8925m, activate: true),
                CreateCurrencyPair("NZDUSD", 4, 5, 0.8263m, activate: true),
                CreateCurrencyPair("EURCAD", 4, 5, 1.5062m, activate: true),
                CreateCurrencyPair("EURAUD", 4, 5, 1.5256m, activate: true),
