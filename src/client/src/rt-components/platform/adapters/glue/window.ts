...
const addFrameButton = (win: GDWindow) => {
  win.addFrameButton(
    ...
    () => {
      win.onFrameButtonClicked((buttonInfo, wnd: GDWindow) => {
        if (!wnd.isCollapsed) {
          wnd.collapse()
          return
        }
