...
export var DockingManager = (function() {
  ...
  function DockingManager() {
    if (instance) {
      throw new Error(
        'Only one instance of DockingManager is allowed. Use DockingManager.getInstance() to get the instance.'
      )
    }
  ...
  DockingManager.prototype.isSnapable = function(currentWidow, window) {
    var isInVerticalZone = this._isPointInVerticalZone(
      window.y,
    ...
    if (
      currentWidow.x > window.x + window.width - currentWidow.currentRange &&
      currentWidow.x < window.x + window.width + currentWidow.currentRange &&
      isInVerticalZone
    ) {
      return 'right'
    } else if (
      currentWidow.x + currentWidow.width > window.x - currentWidow.currentRange &&
      currentWidow.x + currentWidow.width < window.x + currentWidow.currentRange &&
      isInVerticalZone
    ) {
      return 'left'
    } else {
      var isInHorizontalZone = this._isPointInHorizontalZone(
        window.x,
      ...
      if (
        currentWidow.y > window.y + window.height - currentWidow.currentRange &&
        currentWidow.y < window.y + window.height + currentWidow.currentRange &&
        isInHorizontalZone
      ) {
        return 'bottom'
      } else if (
        currentWidow.y + currentWidow.height > window.y - currentWidow.currentRange &&
        currentWidow.y + currentWidow.height < window.y + currentWidow.currentRange &&
        isInHorizontalZone
      ) {
        return 'top'
