...
async function run(
  ...
  try {
    ...
    try {
      ...
      await fsPromises.rm("./reports/screenshots/", {
        recursive: true,
        force: true,
      })
...
setup("start openfin", async () => {
  ...
  //   )
  // })
  const testManifestUrl = "https://openfin.env.reactivetrader.com/pull/2276/config/rt-fx.json"
  const chromeDriverPort = 1917
  const devToolsPort = 9091
  ...
  await run(openFinRVM, testManifestUrl, chromeDriverPort, devToolsPort)
