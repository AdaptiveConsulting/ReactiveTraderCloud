...
const tradeIntentHandler: Handler = (
  symphony,
  { intentsFromDF$ },
  { tradeStream$ },
) => {
  ...
  const subscription$ = intentsFromDF$
    .pipe(
      filter(
        (x) =>
          x.intentResponse.queryResult.intent.displayName ===
          INTENT_TRADES_INFO,
      ),
      withLatestFrom(latestTrades$),
      mergeMap(([request, trades]) => {
        const fields: TradeIntentFields =
          request.intentResponse.queryResult.parameters.fields
        const count =
          fields.number && fields.number.numberValue
            ? fields.number.numberValue
            : 20
        const ccyPair = fields.CurrencyPairs
          ? fields.CurrencyPairs.stringValue
          : ""
        ...
        if (ccyPair) {
          filteredTrades = filteredTrades.filter(
            (x) => x.CurrencyPair === ccyPair,
          )
        }

        if (ccy) {
          filteredTrades = filteredTrades.filter(
            (x) => x.CurrencyPair.substr(0, 3) === ccy,
          )
        }
        ...
        const label = `Here are your last ${count} ${
          field || ""
        } trades as of ${formatDateTime(new Date())}`
